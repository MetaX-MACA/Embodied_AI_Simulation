...
from utils.wrappermujoco import EnvWrapperMujoco
...
def getParser():
    ...
    parser.add_argument('--save_freq', type=int, default=int(1e5), help='# of time steps for save.')
    parser.add_argument('--wandb_freq', type=int, default=int(1e5), help='# of time steps for wandb logging.')
    parser.add_argument('--slack_freq', type=int, default=int(2.5e6), help='# of time steps for slack message.')
    parser.add_argument('--seed', type=int, default=1, help='seed number.')
    parser.add_argument('--task_cfg_path', type=str, default="./tasks/go1_backflip_mujoco.yaml", help='cfg.yaml file location for task.')
    parser.add_argument('--algo_cfg_path', type=str, default="./algos/comoppo/go1_backflip_mujoco.yaml", help='cfg.yaml file location for algorithm.')
...
def train(args, task_cfg, algo_cfg):
    ...
    task_cfg["model_num"] = args.model_num
    task_cfg["test"] = False
    task_cfg['is_uniform_rollout'] = True
    
    env_fn = lambda: task_dict[task_cfg['name']](cfg=task_cfg)
    vec_env = EnvWrapperMujoco(env_fn, task_cfg['env']['num_envs'])
    
    args.device = vec_env.device
    args.n_envs = vec_env.n_envs
    args.max_episode_len = vec_env.unwrapped.get_attr("max_episode_length")[0]
    
    args.num_stages = vec_env.unwrapped.get_attr("num_stages")[0]
    args.obs_dim = vec_env.obs_dim
    args.state_dim = vec_env.unwrapped.get_attr("num_states")[0] - args.num_stages
    args.action_dim = 12 # for go1
    args.reward_dim = vec_env.unwrapped.get_attr("num_rewards")[0]
    args.cost_dim = vec_env.unwrapped.get_attr("num_costs")[0]
    ...
    args.history_len = vec_env.unwrapped.get_attr("history_len")[0]
    args.obs_sym_mat = vec_env.unwrapped.get_attr("obs_sym_mat")[0]
    args.state_sym_mat = vec_env.unwrapped.get_attr("state_sym_mat")[0]
    args.joint_sym_mat = vec_env.unwrapped.get_attr("joint_sym_mat")[0]
    ...
    for _ in range(int(initial_step/args.n_steps), int(args.n_total_steps/args.n_steps)):
        ...
        for _ in range(int(args.n_steps/args.n_envs)):
            ...
            if (total_step % 1000) == 0:
                print("{}/{}".format(total_step, args.n_total_steps))

            # ======= collect trajectories & training ======= #
            with torch.no_grad():
                actions_tensor = agent.getAction(obs_tensor, states_tensor, stages_tensor, False)
                obs_tensor, states_tensor, rewards_tensor, dones, infos = vec_env.step(actions_tensor.cpu().numpy())
                stages_tensor = states_tensor[:, -args.num_stages:]
                states_tensor = states_tensor[:, :-args.num_stages]
...
def test(args, task_cfg, algo_cfg):
    ...
    # used for video save
    task_cfg["model_num"] = args.model_num
    task_cfg["test"] = args.test
    task_cfg['is_uniform_rollout'] = False
    env_fn = lambda: task_dict[task_cfg['name']](cfg=task_cfg)
    vec_env = EnvWrapperMujoco(env_fn, task_cfg['env']['num_envs'])
    
    # set arguments vec_env.unwrapped.get_attr("num_stages")[0]
    args.device = vec_env.device
    args.n_envs = vec_env.n_envs
    args.max_episode_len = vec_env.unwrapped.get_attr("max_episode_length")[0]
    ...
    # declare agent
    # The above code seems to be defining a variable named `agent_args` in Python.
    agent_args = deepcopy(args)
    for key in algo_cfg.keys():
        agent_args.__dict__[key] = algo_cfg[key]
    agent = algo_dict[args.algo_name.lower()](agent_args)
    ...
    for _ in range(1):
        ...
        for step_idx in range(args.max_episode_len):
            with torch.no_grad():
                ...
                obs_tensor, states_tensor, rewards_tensor, dones_tensor, infos = vec_env.step(actions_tensor.cpu().numpy())
                stages_tensor = states_tensor[:, -args.num_stages:]
                states_tensor = states_tensor[:, :-args.num_stages]
                next_obs_tensor = infos['next_obs']
                next_stages_tensor = infos['next_states'][:, -args.num_stages:]

...
# --------------------- Argument Parsing --------------------- #
def parse_args():
    parser = argparse.ArgumentParser(description="Dueling DQN for CartPole-v0")
...
def setup_logging(device: str) -> logging.Logger:
    log_file = f"dueling_dqn_training_{device}.log"
...
def main():
    ...
    q_net = DuelingQNetwork(obs_dim, act_dim).to(args.device)
    target_net = DuelingQNetwork(obs_dim, act_dim).to(args.device)
    ...
    while total_steps < args.total_timesteps:
        ...
        if len(buffer) > args.train_start and len(buffer) >= args.batch_size:
            ...
            with torch.no_grad():
                ...
                target_q = rewards + (1 - dones) * args.gamma * max_next_q
        ...
        if total_steps % args.save_interval == 0:
            save_name = f"dueling_dqn_{args.env_id}_step{total_steps}.pt"

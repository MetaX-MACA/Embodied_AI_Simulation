...
# --------------------- Argument Parsing --------------------- #
def parse_args():
    parser = argparse.ArgumentParser(description="Advantage Actor-Critic (A2C) for CartPole-v0")
...
def setup_logging(device: str) -> logging.Logger:
    log_file = f"a2c_training_{device}.log"
...
def main():
    ...
    while total_steps < args.total_timesteps:
        ...
        for _ in range(args.batch_size):
            ...
            values.append(value)
            rewards.append(torch.tensor([reward], dtype=torch.float32, device=args.device))
            dones.append(torch.tensor([done], dtype=torch.float32, device=args.device))
        ...
        with torch.no_grad():
            next_state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0).to(args.device)
        ...
        if total_steps % args.save_interval == 0:
            ...
            save_name = f"a2c_{args.env_id}_step{total_steps}.pt"

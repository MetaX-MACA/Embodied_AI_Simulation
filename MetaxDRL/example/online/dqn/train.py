...
# --------------------- Argument Parsing --------------------- #
def parse_args():
    parser = argparse.ArgumentParser(description="Deep Q-Network (DQN) for CartPole-v0")
...
def setup_logging(device: str) -> logging.Logger:
    log_file = f"dqn_training_{device}.log"
...
def main():
    ...
    while total_steps < args.total_timesteps:
        ...
        if total_steps % args.save_interval == 0:
            save_name = f"dqn_{args.env_id}_step{total_steps}.pt"
